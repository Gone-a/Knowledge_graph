...
def _get_predict_data(cfg):
    data_file = input('请输入包含测试数据的 CSV 文件路径：')
    try:
        ...
        for index, row in df.iterrows():
            instance = {
                ...
                'head': row['head'].strip(),
                'tail': row['tail'].strip(),
                'head_type': row['head_type'].strip() if pd.notna(row['head_type']) else 'None',
                'tail_type': row['tail_type'].strip() if pd.notna(row['tail_type']) else 'None'
            }
            if instance['head_type'] == '' or instance['tail_type'] == '':
        ...
        print(f'文件 {data_file} 未找到，请检查路径。')
        sys.exit(1)
    except KeyError:
        print('CSV 文件中缺少必要的列（sentence, head, tail, head_type, tail_type）。')
        sys.exit(1)
...
def main(cfg):
    ...
    for i, instance in enumerate(data):
        if cfg.model_name != 'lm':
            x = dict()
            x['word'], x['lens'] = torch.tensor([instance['token2idx'] + [0] * (512 - len(instance['token2idx']))]), torch.tensor([instance['seq_len']])
            x['head_pos'], x['tail_pos'] = torch.tensor(instance['head_pos'] + [0] * (512 - len(instance['token2idx']))), torch.tensor(instance['tail_pos'] + [0] * (512 - len(instance['token2idx'])))
            
            if cfg.model_name == 'cnn':
                if cfg.use_pcnn:
                    x['pcnn_mask'] = torch.tensor([instance['entities_pos']])
            ...
            logger.info(f"第 {i+1} 条数据：\"{instance['head']}\" 和 \"{instance['tail']}\" 在句中关系为：\"{prob_rel}\"，置信度为{prob:.2f}。")
            ...
            logger.info(f"第 {i+1} 条数据：\"{instance['head']}\" 和 \"{instance['tail']}\" 在句中关系为：\"{best_relation}\"，置信度为{max_prob:.2f}。")
